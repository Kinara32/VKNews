...
class NewsfeedPresenter: NewsfeedPresentationLogic {
    ...
    func presentData(response: Newsfeed.Model.Response.ResponseType) {
        switch response {
        case let .presentNewsFeed(feed: feed, revealdedPostIds: revealdedPostIds):
        ...
        case let .presentUserInfo(user: user):
    ...
    private func photoAttachment(feedItem: FeedItem) -> FeedViewModel.FeedCellPhotoAttachment? {
        guard let photos = feedItem.attachments?.compactMap(\.photo), let firstPhoto = photos.first else { return nil }
